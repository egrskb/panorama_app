...
Дополнено:
— Калибровка dBm:
   * libhackrf-режим: загрузка CSV через hq_load_calibration() + включение через hq_enable_calibration(1)
   * hackrf_sweep-режим: Python-LUT с интерполяцией оффсета по частоте
"""
...
# -------------------- Calibration (Python/LUT for hackrf_sweep) --------------------
# LUT калибровки CSV: строки "freq_mhz,lna_db,vga_db,amp,offset_db"
_CAL_LUT = None

def _load_cal_lut(path="hackrf_cal.csv", lna=None, vga=None, amp=0):
    """Загружает LUT оффсетов [MHz,dB] для заданных LNA/VGA/AMP. Возвращает np.array Nx2 или None."""
    ...
    try:
        with open(path, "r", encoding="utf-8") as fh:
            for line in fh:
                ...
                if len(parts) < 5:
                    continue
                f_mhz, lna_db, vga_db, amp_b, off_db = parts[:5]
                try:
                    lna_db = int(lna_db.strip()); vga_db = int(vga_db.strip()); amp_b=int(amp_b.strip())
                    f_mhz = float(f_mhz.strip()); off_db = float(off_db.strip())
                except Exception:
                    continue
                if (lna is None or lna_db==lna) and (vga is None or vga_db==vga) and amp_b==amp:
                    lut.append((f_mhz, off_db))
        if not lut:
            return None
        lut.sort(key=lambda t: t[0])
        return np.array(lut, dtype=np.float64)  # Nx2: [MHz, dB]
    except Exception:
        return None

def _apply_lut(freqs_hz: np.ndarray, p_dbm: np.ndarray, lut_mhz_db: np.ndarray):
    """Возвращает p_dbm + offset(freq) по интерполяции."""
    if lut_mhz_db is None or len(lut_mhz_db)==0:
        return p_dbm
    f_mhz = freqs_hz * 1e-6
    x = lut_mhz_db[:,0]; y = lut_mhz_db[:,1]
    offs = np.interp(f_mhz, x, y, left=y[0], right=y[-1]).astype(np.float32)
    return p_dbm + offs
...
class LibWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        # # === КАЛИБРОВКА (libhackrf) ===
        # cal_path = os.path.join(os.path.dirname(__file__), "hackrf_cal.csv")
        # try:
        #     bpath = cal_path.encode("utf-8")
        # except Exception:
        #     bpath = cal_path
        # if os.path.isfile(cal_path):
        #     r = lib.hq_load_calibration(bpath)
        #     if r == 0:
        #         lib.hq_enable_calibration(1)
        #         self.status.emit(f"Калибровка: {os.path.basename(cal_path)}")
        #     else:
        #         self.status.emit("Калибровка: ошибка загрузки LUT (работаем без неё)")
        # else:
        #     self.status.emit("Калибровка: файл hackrf_cal.csv не найден (без LUT)")
        # # ===============================
...
class SweepWorker(QtCore.QThread):
    ...
    def __init__(self, f_start_mhz=2400.0, f_stop_mhz=2483.0, bin_hz=1_000_000, lna=24, vga=20, serial_suffix=""):
        ...
        self._lut = None  # сюда MainWindow положит LUT (если есть)
    def stop(self):
        self._stop.set()
        try:
    ...
    def _add_segment(self, freqs, power):
        # применяем LUT (если задан) сразу на сегменте
        if self._lut is not None:
            power = _apply_lut(freqs, power, self._lut)
        if self._grid is None or self._n==0: return
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self._cal_lut = None  # LUT для режима hackrf_sweep
    ...
    def on_start(self):
        ...
        if method==0:
            # Разбор hackrf_sweep — загрузим LUT калибровки (если есть) под текущие LNA/VGA
            self._cal_lut = _load_cal_lut("hackrf_cal.csv", self.lnaBox.value(), self.vgaBox.value(), amp=0)
            self.worker = SweepWorker(self.cfg_f0_mhz, self.cfg_f1_mhz, bw, self.lnaBox.value(), self.vgaBox.value(), serial_suffix=serial)
            self.worker._lut = self._cal_lut  # передаём LUT воркеру
            if self._cal_lut is not None:
                self.statusBar().showMessage("Калибровка (hackrf_sweep): применяем LUT из hackrf_cal.csv")
        else:
            self._cal_lut = None
    ...
    def on_sweep(self, f_grid_hz: np.ndarray, p_grid_dbm: np.ndarray):
        ...
        y_proc = p_grid_dbm.copy()  # в hackrf_sweep ветке LUT уже применён в воркере
