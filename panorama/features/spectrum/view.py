...
class SpectrumView(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.water_plot.invertY(True)  # свежие сверху
    ...
    def _build_right_panel(self) -> QtWidgets.QWidget:
        ...
        h = QtWidgets.QHBoxLayout(); h.addWidget(QtWidgets.QLabel("Окно:")); h.addWidget(self.avg_win)
        gl.addRow(h); gl.addRow(self.chk_min); gl.addRow(self.chk_max)
        ...
        self.alpha = QtWidgets.QDoubleSpinBox(); self.alpha.setRange(0.00, 1.00); self.alpha.setSingleStep(0.05); self.alpha.setValue(0.30)
    ...
    def _auto_levels(self):
        ...
        if self._model.water is None:
            return
        data = self._model.water[self._model.water > -200]  # исключаем пустые значения
        if data.size == 0:
            return
    ...
    def _on_full_sweep(self, freqs_hz: np.ndarray, power_dbm: np.ndarray):
        ...
        # Эмитим для других модулей (пики и т.д.)
    ...
    def _snap_bounds(self, f0_mhz: float, f1_mhz: float, bin_hz: float) -> Tuple[float, float, float]:
        ...
        bin_mhz = bin_hz / 1e6
        seg_mhz = 5.0  # типичный шаг сегмента hackrf_sweep
