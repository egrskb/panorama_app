...
from panorama.features.settings.storage import \
    load_detector_settings as load_detector_settings_for_cluster
from panorama.features.settings.storage import save_detector_settings
...
from panorama.shared.rms_utils import (create_target_json,
                                       should_recenter_target)
    ...
    def _load_config(self, config_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if config_file.exists():
                ...
                # Merge cluster settings from detector_settings
                detector_settings = load_detector_settings()
                if "cluster" in detector_settings:
                    config["detector"]["cluster"] = detector_settings["cluster"]

                return config
        except Exception as e:
            print(f"Failed to load config from {config_path}: {e}")

        # Return default configuration with cluster from detector_settings
        detector_settings = load_detector_settings()
        default_cluster = detector_settings.get("cluster", {
            "min_bw_hz": 1500000,
            "max_bw_hz": 12000000,
